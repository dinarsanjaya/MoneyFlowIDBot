...
### Reset Data
Untuk reset semua data (kehapus semua transaksi, user, sheet):
```bash
node reset_all.js
```

⚠️ **Peringatan**: Ini akan menghapus SEMUA transaksi di spreadsheet dan file user data. Pastikan sudah backup sebelumnya!

---

## 🔒 Keamanan

- Semua data sensitif di `.env` (tidak di-commit ke Git)
- Data user disimpan lokal di `data/` folder
- Service account hanya bisa akses spreadsheet yang di-share kepadanya
- Bot hanya merespons user yang sudah `/start`
- File `.gitignore` mencakup: `.env`, `credentials/`, `data/`

---

## 📝 Catatan Penting

- **Sheet otomatis** dibuat setiap awal bulan (cron job jam 00:01)
- **Reminder tagihan** dikirim 3 hari sebelum & pada hari jatuh tempo (jam 09:00)
- **Saldo otomatis** dihitung via formula `SUMIF` di spreadsheet
- **Timezone default**: `Asia/Jakarta` (WIB) — bisa diubah di `.env`
- **Backup spreadsheet** setiap bulan untuk keamanan data

---

## 🚀 Roadmap & Future Features

- [ ] **Recurring Transactions** — Automasi transaksi rutin
- [ ] **Multi-language** — Tambah Mandarin, Jepang, dll
- [ ] **Database** — Migrasi dari JSON ke PostgreSQL/MongoDB
- [ ] **Web Dashboard** — Dashboard web untuk visualisasi lebih baik
- [ ] **Mobile App** — Native app untuk iOS/Android
- [ ] **Budget Notifications** — Alert ketika budget mau habis
- [ ] **Expense Forecasting** — Prediksi spending bulan depan
- [ ] **Investment Tracking** — Tracking portfolio saham & crypto
- [ ] **Bill Automation** — Auto-pay bills & reminder
